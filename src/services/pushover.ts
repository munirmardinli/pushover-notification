...
export class Pushover {
   ...
   */
	private setDefaults(payload: MessagePayload): MessagePayload {
		...
		const stringDefaults: Partial<Pick<MessagePayload, 'device' | 'title' | 'url' | 'url_title' | 'sound'>> = {
			device: '',
			title: '',
			url: '',
			url_title: '',
		...
		for (const key in stringDefaults) {
			if (obj[key as keyof typeof stringDefaults] === undefined) {
				obj[key as keyof typeof stringDefaults] = stringDefaults[key as keyof typeof stringDefaults]!
			}
		}

		const numberDefaults: Partial<Pick<MessagePayload, 'priority' | 'timestamp'>> = {
			priority: 0,
			timestamp: 0,
		}
		...
		for (const key in numberDefaults) {
			if (obj[key as keyof typeof numberDefaults] === undefined) {
				obj[key as keyof typeof numberDefaults] = numberDefaults[key as keyof typeof numberDefaults]!
			}
		}
   ...
   */
	private buildMultipartForm(reqStr: string, boundary: string, image?: ImageFile): Buffer {
		const parts: string[] = []
		...
		for (const [key, value] of Object.entries(parsed)) {
			if (value !== '') {
				parts.push(`--${boundary}`)
				parts.push(`Content-Disposition: form-data; name="${key}"`, '', value as string)
			}
		}

		if (image) {
			parts.push(`--${boundary}`)
			parts.push(`Content-Disposition: form-data; name="attachment"; filename="${image.name}"`)
			parts.push(`Content-Type: ${image.type ?? 'application/octet-stream'}`, '', '')
		}
   ...
   */
	public send(
		...
		const image = typeof form.file === 'string' ? this.loadImage(form.file) : form.file
		...
		const requester = options.protocol?.startsWith('http:') || this.httpOptions?.proxy
			? httpRequest
			: httpsRequest
