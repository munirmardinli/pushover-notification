 ...
 */
	public async create(
		notification: Omit<NotificationData, 'id' | 'read' | 'createdAt' | 'pushoverSent' | 'pushoverReceipt'>
		...
		if (this.pushoverService.isEnabled()) {
			try {
				...
				const receipt = await this.pushoverService.sendNotification(pushoverMessage);
	...
	public createNotification(req: Request, res: Response): void {
		try {
			...
			if (!title || !message || !recipient) {
				res.status(400).json({ error: 'Title, message and recipient are required' });
				return;
			}

			this.notificationStore.create({ title, message, recipient })
...
export class App {
	...
	constructor(port: number, dataFile: string) {
		...
		this.notificationController = new NotificationController(this.notificationStore);
 ...
 */
	public start(): void {
		this.app.listen(this.port, () => {
			...
			console.log(`Pushover service ${this.notificationStore['pushoverService'].isEnabled() ? 'enabled' : 'disabled'}`);
		});
	}
}
